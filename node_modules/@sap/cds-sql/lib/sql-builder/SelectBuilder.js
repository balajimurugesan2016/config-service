...
class SelectBuilder extends BaseBuilder {
  ...
  build (noQuoting = false) {
    ...
    if (this._obj.SELECT.hasOwnProperty('limit') || this._obj.SELECT.hasOwnProperty('one')) {
      this._limit()
    }

    if (this._obj.SELECT.hasOwnProperty('forUpdate')) {
      this._forUpdate()
    }
  ...
  _forUpdate () {
    ...
    const sqls = []
    for (const element of this._obj.SELECT.forUpdate.columns) {
      if (element === '*') {
        break
      }
    ...
    if (this._obj.SELECT.forUpdate.wait && !isNaN(this._obj.SELECT.forUpdate.wait.val)) {
      this._outputObj.sql.push('WAIT', this._obj.SELECT.forUpdate.wait.val)
    }
  }
