...
const _getCompositionTree = ({
  ...
  for (const element of elements) {
    if (_isManaged(element) && _navigationExistsInCompositionMap(element, compositionMap, includeAssociations)) {
...
const createCascadeDeleteCQNs = (definitions, cqn) => {
  ...
  const compositionTree = draft
    ? getDraftCompositionTree(definitions, entityName, false)
    : getCompositionTree(definitions, entityName, false)
...
const hasDeepUpdate = (definitions, cqn) => {
  if (cqn && cqn.UPDATE && cqn.UPDATE.entity && cqn.UPDATE.data) {
    const entityName = cqn.UPDATE.entity.name || cqn.UPDATE.entity
...
const _removeLocalizedTextsFromDraftTree = (compositionTree, definitions, checkedEntities = new Set()) => {
  for (const e of compositionTree.compositionElements) {
    if (checkedEntities.has(e.source)) {
      return
    } else if (e.name === 'texts' && definitions[e.target].elements.localized) {
...
const getDraftCompositionTree = (definitions, rootEntityName, checkRoot, includeAssociations) => {
  const cTree = getCompositionTree(definitions, rootEntityName, checkRoot, includeAssociations)
  _removeLocalizedTextsFromDraftTree(cTree, definitions)
