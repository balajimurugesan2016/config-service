...
function _skip_unused (csn) { // NOSONAR
  ...
  const entities = m.all ('entity')
  const _deferred={}, _pass2={}, _keep={}
  ...
  m.foreach (e => !e.abstract && e['@cds.persistence.skip']=='if-unused', e1 => { // NOSONAR
    ...
    for (let e2 of entities) {
      ...
      // is e2 a view on e1?
      if (e2.query && e2.query.target === e1 && keep (e1, `projection`, e2))  return

      // does e2 contain Associations to e1?
      for (let e in e2.elements) {
        if (e2.elements[e]._target === e1 && keep (e1, `reference '${e}'`, e2))  return
      }
  ...
  for (let each in _pass2) {
    for (let e of _pass2[each])  skip (e[0].name)
  }
