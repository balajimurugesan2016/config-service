...
class HanaModuleBuilder extends BuildTaskHandler {
    ...
    async _compileToHana(model, dest) {
        ...
        const result = this.cds.env.hana.syntax === 'hdi'
            ? this.cds.compile.to.hdi(model)
            : this.cds.compile.to.hana(model)

        for (let [each, { name, suffix }] of result) {
            const relativeDestinationPath = path.join(GENERATED_SOURCES_FOLDER, name + suffix)
            this._result.hana.push(relativeDestinationPath)
