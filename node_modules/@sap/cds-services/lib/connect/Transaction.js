...
class Transaction {
  ...
  _followProjectionOfAll (args) {
    for (const query of args) {
      followProjection(query, this._service)
    }
    return args
  }
  ...
  _addThenableToRun (promise) {
    ...
    then.run = (...args) => {
      return this._addThenableToRun(
        promise.then(() => this._acquireAndExecute('run', ...this._followProjectionOfAll(args)))
      )
    }
  ...
  run (...args) {
    return this._addThenableToRun(this._acquireAndExecute('run', ...this._followProjectionOfAll(args)))
  }
