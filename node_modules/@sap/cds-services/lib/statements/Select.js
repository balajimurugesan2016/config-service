...
class Select extends Where {
   ...
   *
   * @param [columns] columns to be locked, defaults to all columns
   * @param [wait] in seconds, specifies when to return an error if a lock cannot be obtained. Value 0 will result in NOWAIT.
   */
  forUpdate (columns, wait) {
    const cqn = this._getCQN()
    cqn.forUpdate = {
      columns: []
    }

    if (this._isNumber(columns) || this._isNumber(wait)) {
      cqn.forUpdate.wait = { val: wait || columns }
    }

    if (Array.isArray(columns)) {
      for (const column of columns) {
        if (column === '*') {
          cqn.forUpdate.columns = ['*']
          break
        } else if (typeof column === 'string' && column.includes('.')) {
          cqn.forUpdate.columns.push(this._parseElementWithDot(column))
        } else {
          cqn.forUpdate.columns.push(this._parseElement(column))
        }
      }
    } else {
      cqn.forUpdate.columns = ['*']
  ...
  _parseElementWithDot (column) {
    if (column.includes('.{')) {
      return { ref: this._matchInline(column) }
    }
    return { ref: column.split(/\./) }
  }
