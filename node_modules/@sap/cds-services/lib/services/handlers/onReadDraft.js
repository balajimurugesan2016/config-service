...
const _buildOrderBy = (query, outerMostColumns, table) => {
  if (query.SELECT.orderBy) {
    for (const entry of query.SELECT.orderBy) {
      if (entry.ref && !outerMostColumns.some(element => element.as === entry.ref[0])) {
        entry.ref.unshift(table.as)
      }
    }
  }
...
const _allActive = (context, columns) => {
  ...
  _buildOrderBy(context.query, outerMostColumns, table)
