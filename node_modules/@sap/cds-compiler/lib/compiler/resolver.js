...
const {
  queryOps, setProp, forEachDefinition, forEachMember, forEachGeneric, forEachInOrder,
} = require('../base/model');
const {
  addToDict, addToDictWithIndexNo, clearDict, dictLocation,
...
function resolve( model ) {
  ...
  const { resolvePath, defineAnnotations, setAutoExposed }
  ...
  function isAutoExposed( target ) {
    ...
    // return false
    while (setAutoExposed( target, true ) && source) {
  ...
  function initFromColumns( query, specifiedElements ) {
    const elements = specifiedElements || Object.create(null);
    ...
    for (const col of query.columns || [ { val: '*' } ]) {
      ...
      const { id } = col.name;
      const elem = specifiedElements && specifiedElements[id];
      if (elem) {
        if (col.name.$inferred) // prepare columns=null hack for assoc2join when...
          elem.name.$inferred = col.name.$inferred; // ...called from CSN processor
    ...
    if (specifiedElements) {
      for (const name in specifiedElements) {
        const elem = specifiedElements[name];
        if (!elem.$replacement && elem.name) { // no name with redefs
          message( 'query-unspecified-element', elem.name.location, query._main, { id: name },
    ...
    function expandWildcard( elements, location, specifiedElements ) {
      ...
      for (const name in query.$combined) {
        ...
        if (selElem && (!specifiedElements || selElem.$replacement)) {
          ...
          selElem.name.$inferred = '*'; // prepare columns=null hack for assoc2join
          ...
          if (specifiedElements) {
            // Happens more often with "auto-exclude".   TODO: probably need to
            // downgrade message severity to Debug for auto-excluded (and
            // change messageId)
          ...
          elem.viaAll = true;
          if (specifiedElements)
            elem.$replacement = true; // already added via linkToOrigin
          setElementOrigin( elem, navElem, name, location );
        }
      }
      if (specifiedElements)
  ...
  function resolveRefs( art ) {
    ...
    if (art.kind === 'element') {
      finalType( art );
      if (!art['@Core.Computed'] && (
          (art.virtual && art.virtual.val === true) ||
          (art.value && (art.value.op || art.value.literal || art.value.scope === 'param' ||
                         (art.value._artifact && art.value._artifact.kind === 'builtin')))
         )) {
        art['@Core.Computed'] = {
          name: {
  ...
  function resolveTechnicalConfig( art ) {
    ...
    if (!tc)
      return;
    if (!options.hanaFlavor && !options.betaMode && !options.beta && tc.location) {
  ...
  function resolveTarget( art, obj ) {
    ...
    if (obj.onCond) {
      if (!art._main || !art._parent.elements) {
        ...
        resolveExpr( obj.onCond, 'unmanaged', art ); // TODO: extra with $inferred (to avoid messages)?
      }
    }
  ...
  function resolveRedirected( elem, target ) {
    ...
    if (nav && nav.item !== elem.value.path[elem.value.path.length - 1]) {
      message( null, elem.target.location, elem, {},
               // TODO: Better text ?
               'Info',
               origType.onCond
                 ? 'With renamed elements, the ON condition might not be properly rewritten'
                 : 'With renamed elements, the foreign keys might not be properly rewritten' );
      return;
    }
  ...
  function rewriteCondition( elem, assoc ) {
    const nav = (elem._main.query) ? pathNavigation( elem.value ) : { navigation: assoc };
    ...
    if (navigation !== assoc && navigation.origin &&
      ...
      // message( warning`Rewriting associations in a structure is not supported`,
      //    elem.value.location );
      return;
    }
      ...
      if (alias.$navigation) {
        ...
        if (!name) {
          ...
          message( 'rewrite-undefined-key', weakLocation( (elem.target || elem).location ), assoc,
                   ...
                   'Warning', 'Foreign key $(ID) has not been found in target $(ART)' );
          return null;
        }
        item.id = name;
      }
  ...
  function resolveTypeExpr( art, user ) {
    ...
    if (!typeArt)
      return;
    const args = art.typeArguments || [];
    const parameters = typeArt.parameters || [];
  ...
  function resolveExpr( expr, expected, user, extDict, itemMode) {
    ...
    if (expr instanceof Array) {
      ...
      for (const step of expr.path) {
        if (!step || !step._artifact || step._artifact instanceof Array)
          break;
        if (step.namedArgs)
        ...
        if (step.where) {   // TODO: support for $projection, $parameters, ...?
          const assoc = finalType( step._artifact );
