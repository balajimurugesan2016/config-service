...
const relevantOdataOptions = [ 'odataVersion', 'sqlMapping', 'odataFormat' ];
 ...
 */
function attachTransformerCharacteristics(csn, transformation, options, relevantOptionNames) {
  const relevant = {};
 ...
 * @param {string[]} relevantOptionNames Option names that are defining characteristics
 */
function checkPreTransformedCsn(csn, options, relevantOptionNames) {
  if (!csn.meta) {
    // Not able to check
    return;
  }
...
function odataInternal(csn, internalOptions) {
  ...
  attachTransformerCharacteristics(oDataCsn, 'odata', internalOptions, relevantOdataOptions);
...
function cdl(csn, externalOptions = {}) {
  ...
  const { result, options } = backends.toCdlWithCsn(csn, internalOptions);
...
module.exports = {
  odata: publishCsnProcessor(odata),
  cdl: publishCsnProcessor(cdl),
  sql: publishCsnProcessor(sql),
  hdi: publishCsnProcessor(hdi),
  hdbcds: publishCsnProcessor(hdbcds),
  edm: publishCsnProcessor(edm),
  edmx: publishCsnProcessor(edmx),
};
...
/**
 * @param {Function} processor CSN processor
 * @returns {Function} Function that calls the processor and recompiles in case of internal errors
 */
function publishCsnProcessor( processor ) {
