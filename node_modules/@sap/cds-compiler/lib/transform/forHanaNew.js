...
const {
  mergeOptions, getTopLevelArtifactNameOf,
...
const { getUtils, cloneCsn, forEachGeneric,
        ...
        forAllQueries, hasBoolAnnotation
...
function transformForHanaWithCsn(inputModel, options) {
  ...
  const _artifactRef = csnRefs(csn).artifactRef;
  const _inspectRef = csnRefs(csn).inspectRef;
  const addDefaultTypeFacets = transformUtils.getTransformers(csn, options, pathDelimiter).addDefaultTypeFacets;
  ...
  if (!options.forHana.keepStructsAssocs && options.forHana.associations != 'assocs') {
    ...
    let newCsn = translateAssocsToJoinsCSN(csn, options);
    ...
    setProp(csn, 'options', options);
    if(messages)
      setProp(csn, 'messages', messages);
  }

  const { flattenStructuredElement, checkForeignKeys,
  ...
  forEachDefinition(csn, (artifact, artifactName) => {
    ...
    if (artifact.kind == 'entity' || artifact.kind == 'view') {
      ...
      if (options.forHana.names==="quoted"
          && hasBoolAnnotation(artifact, '@cds.persistence.exists')) {
        let firstPath = artifactName.split('.')[0];
        let topParent = csn.definitions[firstPath];
    ...
    if (artifact.kind == 'entity' || artifact.kind == 'view') {
      basicFlattening(artifact, artifactName);
    }
  ...
  forEachDefinition(csn, (artifact, artifactName) => {
    // Do things specific for entities and views (pass 1)
    if (artifact.kind == 'entity' || artifact.kind == 'view') {
      basicAssociationHandling(artifact, artifactName);
    }
  ...
  forEachDefinition(csn, (artifact, artifactName, something, path) => {
    ...
    if(isPartOfService(artifactName) && ['entity', 'view'].includes(artifact.kind) && artifact['@odata.draft.enabled']) {
      let serviceName = getMatchingService(artifactName);
      ...
      if(service.$draftCount > 1)
        signal(info`Service "${serviceName}" should not have more then one draft root artifact`, ['definitions', artifactName]);
    }
  });
    ...
    if (artifact.kind === 'namespace')
      ...
      forEachMemberRecursively(artifact, (member, memberName, prop, path) => {
        ...
        if (!member._ignore && member.type) {
          checkTypeParameters(member, memberName, csn, path);
        }
        if (!member._ignore && member.items && member.items.type) {
          checkTypeParameters(member.items, memberName, csn, path.concat(['items']));
        }
    ...
    function getSources(query){
      if(query.SET){
        ...
        if(query.SELECT.from.args){
          ...
          const art = _artifactRef(query.SELECT.from);
      ...
      function mergeElementsIntoMap(existingMap, elements, $location, parent, error_parent){
        for(const elementName in elements){
          ...
          existingMap[elementName].push({ element: element, name: elementName, source: $location, parent: getBaseName(parent), error_parent: error_parent });
        }
  ...
  function getBaseName(name){
    ...
    while (csn.definitions[name]) {
      if (csn.definitions[name].kind === 'context')
        return parts.join('.');
      lastDotIdx = name.lastIndexOf('.');
      parts.push(name.substring(lastDotIdx + 1));
      if (lastDotIdx === -1) return parts.join('.');
  ...
  function ignoreAssociationToSkippedTarget(member, memberName, prop, path) {
    if(dialect === 'hana' && !member._ignore && isAssocOrComposition(member.type) && isUnreachableAssociationTarget(csn.definitions[member.target])){
  ...
  */
  ...
  function transformEntityOrViewPass2(query, artifact, artName, type, path) {
    const elements = queryOrMain(query, artifact).elements;
    ...
    for (let elemName in elements) {
      ...
      if (isSelect) {
        ...
        if (elem.keys && !options.forHana.keepStructsAssocs) {
          ...
          if (assoc_col && assoc_col.ref) {
            elem.keys.forEach(key => {
              ...
              ref[ref.length-1] = [ref[ref.length-1]].concat(key.ref).join(pathDelimiter);
              const result = {
                ref: ref
              };
              if(assoc_col.as){
        ...
        if(isStructured(elem) && !options.forHana.keepStructsAssocs){
          ...
          const flatElements = flattenStructuredElement(elem, originalName);
          const aliasedFlatElements = originalName !== elemName ? Object.keys(flattenStructuredElement(elem, elemName)) : [];
          ...
          if (mixinElem.on) {
            mixinElem.on = cloneWithTransformations(mixinElem.on, {
              ref: (ref) => {
                ...
                let clonedPath = cloneWithTransformations(ref, {});
    ...
    if(isSelect){
      ...
      for(let i = 0; i < query.SELECT.columns.length; i++){
        ...
        if(!col.as && col.ref && col.ref.length > 1){
          let { links } = inspectRef(path.concat(['columns',i]));
  ...
  function collectDraftNodesInto(artifact, artifactName, rootArtifact, draftNodes) {
    ...
    for (let elemName in artifact.elements) {
      ...
      if (elem.target && isComposition(elem.type)) {
        let draftNode = getCsnDef(elem.target);
  ...
  function generateDraftForHana(artifact, artifactName, draftRootName) {
    ...
    let draftsArtifactName = artifactName + '_drafts';
    ...
    let keyNames = Object.keys(artifact.elements).filter(elemName => {
      return artifact.elements[elemName].key
    });

    if(artifactName === draftRootName) {
      // draft root
      if(keyNames.length != 1)  {
        ...
        if (keyElem.type !== 'cds.String' && keyElem.$renamed != 'cds.UUID' && keyElem.length !== 36) {
    ...
    // draft node
      if(keyNames.length < 1) {
        ...
        signal(info`"${artifactName}": "@odata.draft.enabled" - Draft node should have not more then two key elements`, ['definitions', artifactName]);
      } else {
        let uuidCount = 0;
        keyNames.forEach(kn => {
    ...
    let draftAdminDataProjectionName = matchingService + '.' + 'DraftAdministrativeData';
    ...
    if (csn.definitions[draftsArtifactName]) {
      let definingDraftRoot = csn.definitions[draftsArtifactName].$draftRoot;
    ...
    // Copy all elements
    for (let elemName in artifact.elements) {
      let elem = copyAndAddElement(artifact.elements[elemName], draftsArtifact, elemName)[elemName];
     ...
     *
     * @param {any} association Assoc to check
     * @returns {Boolean}
     */
    ...
    if (!options.forHana.keepStructsAssocs && draftUUIDKey) {
      let path = ['definitions',draftsArtifactName,'elements','DraftAdministrativeData','keys',0]
  ...
  function redirectDraftTargets(artifact, draftNodes) {
    for (let elemName in artifact.elements) {
      ...
      if (elem.target) {
        ...
        let { shadowTarget, shadowTargetName } = getDraftShadowEntityFor(targetArt, elem.target);
        // Might not exist because of previous errors
        if (shadowTarget) {
  ...
  /**
   * Return the foreign keys for an association in flat format
   *
   * Replaces managed assocs with their respective keys - recursively
   *
   * @param {any} element
   ...
   * @returns
   */
  function getFlatForeignKeys(element, elementName, path){
    const keys = [];
    for(let i = 0; i < element.keys.length; i++){
      const { art, links } = inspectRef(path.concat(['keys', i]));
      const ref = element.keys[i].ref;
      if(art.target){
        replaceAssociationRefWithFKs(element.target, art, ref[ref.length-1], links)
        .forEach(key => {
          const clone = cloneCsn(element.keys[i]);
          if(key.ref){
            ...
            clone.ref.push(key.ref[0]);
          }
          keys.push(clone);
        });
      } else if(isStructured(art)) {
        const flat = flattenStructuredElement(art, ref[ref.length-1]);
        ...
        const clone = cloneCsn(element.keys[i]);
        if(!clone.as && ref.length > 1){
          clone.as = ref[ref.length-1];
          // TODO: can we use $inferred? Does it have other weird side-effects?
          setProp(clone, '$inferredAlias', true);
        }
        keys.push(clone);
   ...
   * @returns
   */
  function replaceAssociationRefWithFKs(originalTarget, assoc, assocName, links){
    // Find out where we currently are so that we can build the path correctly
    // Because we need to resolve refs
    let lastArtName = originalTarget;
    for(let k = 0; k < links.length-1; k++){
      if(links[k].target){
        lastArtName = links[k].target;
      }
    }
    return getFlatForeignKeys(assoc, assocName, ['definitions', lastArtName, 'elements', assocName]);
  }
  ...
  function checkQueryForNoDBArtifacts(query, path){
    ...
    function checkRef(obj, path, inFrom){
      ...
      const { links, scope } = _inspectRef(path);
  ...
  function checkTypeParameters(node, nodeName, model, path) {
    if (node.type) {
      let absolute = node.type;
   ...
   *
   * @param {any} artifactName Absolute name of the artifact
   * @returns  {Boolean}
   */
  ...
  function basicFlattening(art, artName) {
    ...
    if (!options.forHana.keepStructsAssocs) {
      ...
      forEachGeneric(art, 'elements', (element, elemName) => {
        ...
        if (isManagedAssociationElement(element)) {
          ...
          if (element.keys) {
            element.keys = getFlatForeignKeys(element, elemName, ['definitions', artName, 'elements', elemName]);
          }
        }
      });
    ...
    if (!options.forHana.keepStructsAssocs) {
      if (tc && tc[dialect]) {
        ...
        for (let name in tc[dialect].indexes) {
          let index = tc[dialect].indexes[name];
          if (Array.isArray(index)) {
            let flattenedIndex = [];
            let isFulltextIndex = (index[0] === 'fulltext');
            index.forEach((val, idx) => {
              if (typeof val === 'object' && val.ref) {
                // Replace a reference by references to it's elements, if it is structured
                let path = ["definitions", artName, "technicalConfig", dialect, "indexes", name, idx];
                let art = inspectRef(path).art;
                if (!art) {
                  ...
                  let refPath = flattenStructStepsInRef(val.ref, path);
                  // ... and take this as the prefix for all elements
                  let flattenedElems = flattenStructuredElement(art, refPath);
                  Object.keys(flattenedElems).forEach((elem, i, elems) => {
                    // if it's not the first entry, add a ',' ...
                    if (i) flattenedIndex.push(',')
   ...
   */
  function basicAssociationHandling(art, artName){
    for (let elemName in art.elements) {
      let elem = art.elements[elemName];
      ...
      if (!options.forHana.keepStructsAssocs) {
        if (isManagedAssociationElement(elem)) {
          ...
          for(let i = 0; i < elem.keys.length; i++){
            ...
            createForeignKeyElement(elem, elemName, foreignKey, art, artName, ['definitions', artName, 'elements', elemName, 'keys', i]);
